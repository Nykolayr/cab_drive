...
| `feature-feasibility-research.mdc` | **Research до плана и кода** — ограничения платформы, README плагинов, known issues |
| `agent-prompts-in-chat.mdc` | Промпты для других агентов — **только в чате**, не `*-prompt.md` в репо |
| `use-existing-ui.mdc` | **Без AlertDialog** — `showConfirmActionSheet`, `FFButtonWidget`, bottom sheets как в Stripe |
