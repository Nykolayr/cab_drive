...
**Автоподтверждение:** сервер после создания выплаты вызывает `POST .../payments/confirm` (если Jump отдал «нужно подтверждение»). На текущем OpenAPI-ключе маршрут **404** — в кабинете Jump включена опция «Open API → требующие подтверждения» без API confirm (см. базу знаний Jump). Нужно **один раз** снять галочку в [my.jump.finance](https://my.jump.finance) (Настройки → Выплаты → Open API) **или** включить интеграцию с confirm в API.

Зависшая #182024002 (12.09): без доступа в кабинет Jump агентом не закрыть; **деньги на карту не ушли**, balance PG **200 ₽** — корректно.
